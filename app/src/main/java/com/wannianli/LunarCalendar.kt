...
object LunarCalendar {
    ...
    // 低12位为12个月大小（1=30天，0=29天），第12位为闰月大小，第16-13位为闰月月份（0表示无闰月）
    private val lunarInfo = intArrayOf(
        ...
        0x0c960,0x0d4a8,0x0d950,0x06550,0x07456,0x0ea50,0x06b20,0x0ab48,0x04ab0,0x0aae0,
        0x0a2e0,0x0d2e3,0x0c960,0x0d557,0x0d4a0,0x0c8c0,0x0b5a0,0x14573,0x052b0,0x0a930,
        0x07954,0x06aa0,0x0ad50,0x055b2,0x04b60,0x0a570,0x05270,0x052f2,0x0d160,0x0e8a8,
        0x0d950,0x05b52,0x056a0,0x096d0,0x04dd5,0x04ad0,0x0a4d0,0x0d4d4,0x0d250,0x0d558,
        ...
        0x0ab60,0x096d5,0x092e0,0x0c960,0x0d4a8,0x0d950,0x06550,0x07456,0x0ea50,0x06b20,
        0x0ab48,0x04ab0,0x0aae0,0x0a2e0,0x0d2e3,0x0c960,0x0d557,0x0d4a0,0x0c8c0,0x0b5a0,
        0x14573,0x052b0,0x0a930,0x07954,0x06aa0,0x0ad50,0x055b2,0x04b60,0x0a570,0x05270,
        0x052f2,0x0d160,0x0e8a8,0x0d950,0x05b52,0x056a0,0x096d0,0x04dd5,0x04ad0,0x0a4d0,
    ...
    private fun lunarYearDays(year: Int): Int {
        val info = lunarInfo[year - BASE_YEAR]
        var sum = 0
        for (m in 1..12) {
    ...
    private fun lunarMonthDays(year: Int, month: Int): Int {
        val info = lunarInfo[year - BASE_YEAR]
        return if ((info shr (month - 1)) and 1 == 1) 30 else 29
    }
    ...
    private fun leapMonthDays(year: Int): Int {
        val info = lunarInfo[year - BASE_YEAR]
        return if ((info shr 12) and 1 == 1) 30 else 29
    }
    ...
    private fun lunarToSolar(lunarYear: Int, lunarMonth: Int, lunarDay: Int, isLeap: Boolean): Calendar {
        ...
        } else if (month > 1 && leap in 1 until month) {
